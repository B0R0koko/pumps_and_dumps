...
from feature_pipes.transform import PumpEvent
...
def transform_features(df: pl.DataFrame, pump_event: PumpEvent, window: str) -> pd.DataFrame:
    ...
    df_features = df_features.resample(
        on="time", rule="1h", offset=timedelta(
            minutes=pump_event.pump_time.minute,
            seconds=pump_event.pump_time.second
        )
    ).first()
