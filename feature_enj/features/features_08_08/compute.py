...
from features.features_08_08.features import *
...
def transform_to_features(
    ...
    for offset in hour_offsets:
        ...
        hourly_features[f"log_return_std_{offset}h_30d"] = calc_scaled_log_return_std(
            df_window=df_window, df_hourly_lr=df_hourly_lr
        )
    ...
    for offset in hour_offsets:
        ...
        imbalance_features[f"quote_imbalance_ratio_{offset}h"] = calc_quote_imbalance_ratio(df_window=df_window)
        # Imbalance ratio in slippages to see if there is skew towards long slippages
        imbalance_features[f"quote_slippage_imbalance_ratio_{offset}h"] = calc_quote_imbalance_ratio(df_window=df_window)
    ...
    for offset in hour_offsets:
        ...
        liquidity_features[f"linear_liquidity_slope_{offset}h"] = calc_linear_liquidity_slope(df_window=df_window)
    ...
    for offset in daily_offsets:
        ...
        res = regress_proba_slippage_on_quote_abs(df_window=df_window)

        liquidity_proba_features[f"has_slippage_slope_{offset}d"] = res[1] if res else None
        liquidity_proba_features[f"has_slippage_intercept_{offset}d"] = res[0] if res else None
