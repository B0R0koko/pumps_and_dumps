...
def transform_to_features(
    ...
    hour_offsets: List[int] = [1, 3, 6, 12, 24, 48, 60, 72, 5*24, 7*24, 14*24]
    ...
    for offset in hour_offsets:
        ...
        hourly_features[f"volume_qty_abs_zscore_{offset}h_30d"] = (
            (df_window["volume_qty_abs"].mean() - df_hourly_lr["volume_qty_abs"].mean()) / df_hourly_lr["volume_qty_abs"].std()
        )
        ...
        hourly_features[f"log_return_std_{offset}h_30d"] = df_window["log_return"].std() / df_hourly_lr["log_return"].std()
    ...
    df_hourly_candles_72h: pd.DataFrame = df_hourly_candles[
        df_hourly_candles["time"] >= time_ub - timedelta(hours=72)
    ].copy()
    ...
    for offset in hour_offsets:
        ...
        # Share of overall slippages of this time window in 72hours
        slippage_features[f"quote_slippage_abs_share_{offset}h_72h"] = (
            df_window["quote_slippage_abs"].sum() / df_hourly_candles_72h["quote_slippage_abs"].sum()
        )
        # Share of long slippages of this time window in 72hours
        slippage_features[f"quote_slippage_abs_long_share_{offset}h_72h"] = (
            df_window["quote_slippage_abs_long"].sum() / df_hourly_candles_72h["quote_slippage_abs_long"].sum()
        )
    ...
    quote_abs_kurt: float = df_trades["quote_abs"].kurt()
    ...
    for offset in hour_offsets:
        ...
        evt_features[f"quote_abs_kurt_{offset}h_30d"] = df_window["quote_abs"].kurt() / quote_abs_kurt
